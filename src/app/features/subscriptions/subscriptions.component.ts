               ...
               <div class="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center relative z-10">
                 <div>
                   <div class="flex items-center gap-2 mb-1">
                      ...
                      @if (sub.maxEntries) {
                        <span class="text-[10px] bg-gray-700 px-2 py-0.5 rounded text-gray-300">Ingressi Limitati</span>
                      } @else {
                 ...
                 <div class="text-right flex flex-col items-end">
                    ...
                    <div class="flex gap-2 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button class="text-xs text-red-400 hover:text-red-300 underline" (click)="deleteSub(sub.id)">Elimina</button>
                    </div>
                 </div>
               </div>
               
               <!-- Decorative bg element -->
               <div class="absolute -right-6 -bottom-6 w-24 h-24 bg-accent/5 rounded-full blur-xl pointer-events-none"></div>
             </app-card>
           } @empty {
          ...
          <div class="lg:col-span-1">
            <app-card [headerTemplate]="true" title="Nuovo Piano">
              <form [formGroup]="form" (ngSubmit)="onSubmit()" class="flex flex-col gap-4">
                 <app-input 
                   label="Nome Piano" 
                   placeholder="Es. Open Semestrale"
                 ...
                 <app-input 
                   label="Descrizione" 
                   placeholder="Include accesso sala pesi..."
                   formControlName="description"
                 ></app-input>

                 <div class="p-3 bg-gray-800/50 rounded border border-gray-700">
                   <label class="flex items-center gap-2 cursor-pointer">
                     <input type="checkbox" formControlName="isLimited" class="w-4 h-4 rounded border-gray-600 text-accent focus:ring-accent bg-gray-700">
                 ...
                 <div class="flex gap-2 mt-2">
                   ...
                   <app-button type="submit" [loading]="loading" [fullWidth]="true" [disabled]="form.invalid">Crea Piano</app-button>
                 </div>
              </form>
            </app-card>
          </div>
